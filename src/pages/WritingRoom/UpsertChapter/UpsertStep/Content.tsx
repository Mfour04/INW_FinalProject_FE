...
export const Content = ({
  ...
  const editor = useEditor({
    ...
    onUpdate({ editor }) {
      setChapterForm({ ...chapterForm, content: editor.getHTML() });
    },
    editorProps: {
      attributes: {
        class: [
    ...
    <section ref={sectionRef} className="rounded-2xl p-2 py-2 px-3">
      <header className="mb-3 pb-2">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 grid place-items-center rounded-xl bg-zinc-100 ring-1 ring-zinc-200 text-zinc-800 dark:bg-white/10 dark:ring-white/10 dark:text-white/90">
      ...
      {/* Sticky bar */}
      <div className="sticky top-0 z-30 -mx-3 px-3 py-2 backdrop-blur-xl border-b bg-white/70 border-zinc-200 supports-[backdrop-filter]:bg-white/60 dark:border-white/10 dark:bg-[#0e1014]/60 dark:supports-[backdrop-filter]:bg-[#0e1014]/40">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 min-w-0">
            <div
              className={[
                "hidden sm:block h-2 rounded-full overflow-hidden ring-1",
                "bg-zinc-200 ring-zinc-300",
                "dark:bg-white/[0.06] dark:ring-white/10",
                "transition-[width] duration-300 ease-out",
              ].join(" ")}
              style={{ width: `${barPx}px`, willChange: "width" }}
            >
              <div
                className={[
                  "h-full transition-[width] duration-250 ease-out",
                  nearLimit ? "bg-rose-500" : "bg-orange-500",
                  "dark:bg-rose-500/85 dark:aria-[ok=true]:bg-orange-500/85",
                ].join(" ")}
            ...
            <span
              className={[
                ...
                "ring-1 bg-zinc-100 text-zinc-700 ring-zinc-300",
                "dark:ring-white/10 dark:bg-white/[0.05] dark:text-white/90",
      ...
      <style>{`
        .ProseMirror { position: relative; color: inherit; }
        .tiptap-placeholder{
          position:absolute; left:16px; top:12px; pointer-events:none;
          font-size:14px;
          color: rgba(0,0,0,0.38);
        }
        .dark .tiptap-placeholder{ color: rgba(255,255,255,0.45); }
      `}</style>

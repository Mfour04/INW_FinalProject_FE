        ...
        <div className="max-w-6xl mx-[50px] p-4 text-white">
            <div className="flex flex-col md:flex-row gap-4 ">
                ...
                <div className="flex flex-wrap gap-7 mt-10 h-[37px]">
                    ...
                    <button onClick={() => navigate(`/novels/writing-room/${novelId}/upsert-chapter`)} className="flex items-center justify-center gap-2.5 bg-[#ff6740] w-[228px] hover:bg-orange-600 px-4 py-1 rounded text-[18px]"><BookMark sx={{ height: '20px', width: '20px' }} /><p>Chương mới</p></button>
            ...
            <div className="flex gap-6 text-[20px] h-[44px] mt-6 mb-4">
                <button onClick={() => setTab('Chapter')} className={`cursor-pointer hover:bg-gray-800 flex items-center justify-center rounded-[10px] ${tab === 'Chapter' ? 'bg-[#2e2e2e]' : undefined} w-[263px]`}>Đã Đăng</button>
                <button onClick={() => setTab('Draft')} className={`cursor-pointer hover:bg-gray-800 flex items-center justify-center rounded-[10px] ${tab === 'Draft' ? 'bg-[#2e2e2e]' : undefined} w-[263px]`}>Bản Nháp </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mt-[25px]">
                {
                    chapterData?.filter(chapter => tab === 'Draft' ? chapter.is_draft : !chapter.is_draft).map(chapter => (
                        <div key={chapter.id} className="h-[72px] rounded cursor-pointer hover:bg-gray-700 transition-colors duration-200">
                            <div className="flex items-center h-full px-4 border-b-2 border-[#d9d9d9] mr-10 justify-between">
                                <div className='flex items-center'>
                                {chapter.chapter_number ? <h1 className="w-[60px] text-[20px]">{chapter.chapter_number}</h1> : undefined}
                                    <div className="ml-2">
                                ...
                                <button onClick={() => navigate(`/novels/writing-room/${novelId}/upsert-chapter/${chapter.id}`)} className='cursor-pointer'>
                                    <ModeEdit />
                                </button>
                            </div>
                        </div>

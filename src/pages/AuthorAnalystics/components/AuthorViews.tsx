...
import { ArrowLeft, Calendar, Eye, BarChart3, ListFilter, Coins, RotateCcw } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { Shell, Container, Card, SoftInput, KpiPill, AreaChart, ChartToolbar } from "./AnalyticsUI";
import { fetchViewsSeries, fetchTopViews, fetchTopChapterViews, fetchReferrers, type ViewPoint } from "../analyticsMock";
...
/* Segmented toggle — nằm trong header, sát bên phải */
...
export default function AuthorViews({
  mode,
  onChangeMode,
}: {
  mode: Mode;
  ...
  const [vFrom, setVFrom] = React.useState<string | undefined>(undefined);
  const [vTo, setVTo] = React.useState<string | undefined>(undefined);
  const [vGran, setVGran] = React.useState<Granularity>("day");

  const [series, setSeries] = React.useState<ViewPoint[]>([]);
  const [topViews, setTopViews] = React.useState<{ novelId: string; title: string; views: number }[]>([]);
  const [topChapters, setTopChapters] = React.useState<{ id: string; title: string; novelTitle: string; views: number }[]>([]);
  const [referrers, setReferrers] = React.useState<{ source: string; views: number }[]>([]);
  ...
  const seriesViews = React.useMemo(() => aggregateSeries(series as any[], "ts", ["views"], vGran), [series, vGran]);
  const totalViews = seriesViews.reduce((s:any,x:any)=>s+x.views,0);
  const avgViewsPerBucket = Math.round(totalViews / Math.max(1, seriesViews.length));
    ...
    <Shell>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                <div className="flex items-center gap-2 px-3 h-9 rounded-xl">
                  <SoftInput type="date" value={vFrom ?? ""} onChange={(e) => setVFrom(e.target.value || undefined)} />
                </div>
                <span className="text-white/40 text-sm">-</span>
                <div className="flex items-center gap-2 px-3 h-9 rounded-xl">
                  <SoftInput type="date" value={vTo ?? ""} onChange={(e) => setVTo(e.target.value || undefined)} />
                </div>
              </div>
      ...
      <Container className="pb-12 space-y-6">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <KpiPill label="Tổng view" value={totalViews.toLocaleString()} icon={<Eye className="h-4 w-4" />} />
          <KpiPill label={`TB/${vGran === "day" ? "ngày" : vGran === "month" ? "tháng" : "năm"}`} value={avgViewsPerBucket.toLocaleString()} icon={<BarChart3 className="h-4 w-4" />} />
          <KpiPill label="Khoảng thời gian" value={renderRange(vFrom, vTo)} icon={<Calendar className="h-4 w-4" />} />
        </section>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            <Card className="p-0 overflow-hidden">
              ...
              <ul className="divide-y divide-white/10">
                {topViews.map((t, i) => (
                  <li key={t.novelId} className="px-3 py-2 flex items-center justify-between">

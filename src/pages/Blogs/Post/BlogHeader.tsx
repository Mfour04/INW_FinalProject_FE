        ...
        <div className="flex justify-between">
            {(["all", "following"] as Tabs[]).map((t) => (
                <div
                    ...
                    className={`relative w-1/2 text-center px-4 py-3 rounded-[10px] text-lg font-semibold cursor-pointer transition-all duration-300 ${tab === t ? "text-white" : "text-[#cecece] hover:text-white"
                        }`}
                >
                    <span
                        className={`transition-all duration-300 ${tab === t ? "text-white text-xl font-bold" : "text-base"
                            }`}
                    >
